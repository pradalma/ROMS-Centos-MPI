...
Make gets its instructions from a description file, by default named makefile or Makefile. This file is also called the makefile, but other files can be used by invoking make with the -f option, e.g.:
...
To clean up, type "make clean" and the clean target will be brought up to date. clean has no dependencies. What happens in that case is that the command rm -f *.o core will always be executed.
...
Now when we change computers, we only have to change the compiler name in one place. Likewise, if we want to try different optimization levels, we only specify that in one place. Notice that you can use comments by starting the line with a #.
Implicit Rules
...
In conclusion, make is a very powerful tool. The book to read is Managing Projects with GNU Make by Robert Mecklenburg, 2005, O'Reilly.

Make has a portable subset of features, with system-dependent extensions. If you want to use extensions, I suggest sticking with those supported by GNU make (gmake), since it is available most everywhere. The newest ROMS versions make use of the power of gmake and depend on it to be the make used. These more advanced features are described in gmake.
